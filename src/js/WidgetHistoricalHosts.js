 ...
 * https://github.com/rockneurotiko/widget-historicalhosts
 *
 * Copyright (c) 2016 CoNWeT
 * Licensed under the Apache-2.0 license.
 */

/* exported WidgetHistoricalHosts */
var WidgetHistoricalHosts = (function () {
    ...
    var WidgetHistoricalHosts = function WidgetHistoricalHosts () {
        this.token = "";
        this.chart = null;
        this.fulldata = {};
        this.limits = true;
    ...
    var drawHost = function drawHost(region, host) {
        if (downloadingData[region][host]) {
            return;
        }

        getHostData.call(this, region, host, function (data) {
            // Check that is still checked
            var regions = $("#region_selector").val() || [];
            var selected = regions.filter(x => x === region).length > 0;
            if (selected) {
                var width = $(document).width();
                drawHorizon.call(this, data, region, width, host);
            }
    ...
    var drawHorizon = function drawHorizon(fullData, region, width, host) {
        ...
        $("<div></div>", {class: "regionTitle", text: region + ": " + host})
            .appendTo($("#" + id));
    ...
    var viewRegion = function viewRegion() {
        if (!this.singleregion || !this.singlehost) {
            return;
        }

        var fullData = this.fulldata[this.singleregion][this.singlehost];
        if (!fullData) {
            return;
        }
        ...
        this.chart = viewSingleRegion.call(this, fullData, this.singleregion + " - " + this.singlehost, multi, limit);
    };

    var getHostData = function getHostData(region, host, next, nexterror) {
        const rurl = url + region + "/hosts/" + host;
        next = next || function () {};
        ...
        const options = {
            ...
            onSuccess: response => {
                ...
                this.fulldata[region][host] = fixedData;
            ...
            onComplete: function () {
                downloadingData[region][host] = false;
            }
        };

        downloadingData[region][host] = true;
        ...
        var resizeHorizonDebounced = debouncer(function () {
            ...
            for (var region in regions) {
                if (regions.hasOwnProperty(region)) {
                    // for regions, draw host
                    for (var host in regions[region]) {
                        if (regions[region].hasOwnProperty(host)) {
                            // clear
                            $("#" + region + "-" + host).remove();
                            drawHorizon.call($this, regions[region][host], region, width, host);
                        }
                    }
                }
            }
